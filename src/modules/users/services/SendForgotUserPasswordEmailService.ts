...
class SendForgotUserPasswordEmailService {
  ...
  async execute({ email }: IRequest): Promise<void> {
    ...
    const { id: tokenId } = await this.userTokensRepository.generate(user.id);
    ...
    await this.mailProvider.sendMail({
      ...
      subject: '[LAYBACK] Recuperação de senha',
      templateData: {
        file: forgotPasswordTemplate,
        variables: {
          name: user.name,
          link: `${process.env.APP_WEB_URL}/reset-password?token=${tokenId}`,
        },
      },
    });
  }
